...
t1 = linspace(0,1000,1000); tspan1 = t1;
y0a(N^3+N^2+N,1)=0; y0a(1:N,1)=xb0; I6 = eye(6);
y0a(N+1:N+N^2,1)= [I6([1:N^2])]';
options1 = odeset('RelTol',1e-8,'AbsTol',1e-8*ones(N^3+N^2+N,1));
[Tstma,Ystma] = ode45(@STTNint4,tspan1,y0a,options1);

%[Tstmfg,Ystmfg] = FGSTTs(xb0,t1);
...
err1=abs(Ystmfg(:,N+1:N+N^2)-Ystmfg1(:,N+1:N+N^2));
figure
plot(Tstma,err1);
title('Error Between Numerical and Analytical Solution: First Order STM');
