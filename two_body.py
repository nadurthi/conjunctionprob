...
def twobodydynamics_3Dcart_noJ2(t,x):
	R=np.norm(x[0:3]);
	rdot=np.zeros((6,1));
	...
	rdot[1]=r[4];
	rdot[2]=x[5];
	rdot[3]=-mu*x[0]/R^3+ad_J2[0]+adrag[0];
	rdot[4]=-mu*x[1]/R^3+ad_J2[1]+adrag[1];
	rdot[5]=-mu*x[2]/R^3+ad_J2[2]+adrag[2];
...
class TwoBody(object):
	def __init__(self,dyn_meth='twobodydynamics_3Dcart_noJ2',J2=None):
		self.J2=None
		self.dyn_meth=dyn_meth

	def integrate(self,t0,tf,Tvec):
		if Tvec[0]!=t0:
			raise Exception("Tvec[0] and t0 have to be the same")
		if Tvec[-1]!=tf:
			raise Exception("Tvec[-1] and tf have to be the same")
